...
class User(ABC):
    def __init__(self,name,phone,username,email,password,role) -> None:
        self.email=email
...
class Shop(User,Product):
    def __init__(self, name, phone, username, email, password, role) -> None:
        super().__init__(name, phone, username, email, password, role)
        self.user=[]
        self.products=[]
        self.cart=[]
        self.login=[]
    ...
    def sign_up_user(self,name,phone,email,username,password,role):
        user=User(name,phone,email,username,password,role)
        self.login[email]=password
        self.login[username]=password
        self.user.append(user)
        if role == 'customer':
            print('\n Signup Successfully as Customer.')
            print(f'\nUser info: {username}\nName: {name}\nPhone:{phone}\nEmail:{email}\nBalance:{user.wallet} TK')
            
    def login(self,email_or_username_password,password):
        if self.login[email_or_username_password]==password:
            for id in self.user:
                if id.email==email_or_username_password or id.username==email_or_username_password:
